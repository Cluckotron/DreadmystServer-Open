    ...
    , m_playedTime(info.playedTime)
{
    ...
    // 100 is Dreadmyst's normal movement-speed percentage.
...
{
    // Server-authoritative movement. The client supplies a requested destination,
    // but only this simulation advances the persisted/server position.
    if (m_isMoving && m_movePathIndex < m_movePath.size() && !isDead() && !isStunned() && !isRooted())
    {
        const float oldX = getX();
        const float oldY = getY();
        float remaining = getMovementSpeed() * std::max(0.0f, deltaTime);

        while (remaining > 0.0f && m_movePathIndex < m_movePath.size())
        {
            const auto& target = m_movePath[m_movePathIndex];
            const float dx = target.x - getX();
            const float dy = target.y - getY();
            ...
            {
                setPosition(getX() + (dx / dist) * remaining,
                            getY() + (dy / dist) * remaining);
                remaining = 0.0f;
            }
        }
        ...
        {
            markDirty();
            sWorldManager.onPlayerMoved(this, oldX, oldY);
        }

        if (m_movePathIndex >= m_movePath.size())
        {
            clearMovementPath();
        }
    }
    else if (m_isMoving && (isDead() || isStunned() || isRooted()))
    {
        clearMovementPath();
    }
...
void Player::setMovementPath(const std::vector<Geo2d::Vector2>& path)
{
    m_movePath = path;
    m_movePathIndex = 0;
    m_isMoving = !m_movePath.empty();
}

void Player::clearMovementPath()
{
    m_movePath.clear();
    m_movePathIndex = 0;
    m_isMoving = false;
...
{
    // Logical world cells per second. MoveSpeedPct is a percentage modifier.
    // This base is intentionally centralized so it can be tuned after the first
    // graphical-client timing test without changing authority semantics.
    constexpr float BASE_MOVE_SPEED = 4.0f;
    int32_t pct = getVariable(ObjDefines::Variable::MoveSpeedPct);
    if (pct <= 0)
        pct = 100;
    pct = std::clamp(pct, 10, 400);
    return BASE_MOVE_SPEED * (static_cast<float>(pct) / 100.0f);
}
...
{
    const float dx = destX - getX();
    const float dy = destY - getY();

    // Any real movement should update facing. Dreadmyst's A/D screen-axis
    // steps use +/-0.75 on both world axes, so the old 1.0-per-axis threshold
    // incorrectly left those directions facing whatever way the player had
    // previously been looking.
    constexpr float ORIENTATION_EPSILON = 0.0001f;
    if ((dx * dx + dy * dy) <= (ORIENTATION_EPSILON * ORIENTATION_EPSILON))
        return;

    // Entity::setOrientation canonicalizes atan2's [-pi, +pi] result into
    // [0, 2pi), matching the released client's direction renderer.
    setOrientation(std::atan2(dy, dx));

    // Broadcast the canonical orientation to visible players.
