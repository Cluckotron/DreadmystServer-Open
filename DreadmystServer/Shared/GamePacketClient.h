...
{
    // Names/order match the released r1189 client source. In Private Protocol v1
    // m_userToken contains local credentials (username:password), not a Dreadmyst web token.
    std::string m_userToken;
    int32_t m_build = 0;
    std::string m_fingerprint;

    uint16_t getOpcode() const override { return Opcode::Client_Authenticate; }
    void pack(StlBuffer& buf) const override { buf << m_userToken << m_build << m_fingerprint; }
    void unpack(StlBuffer& buf) override { buf >> m_userToken >> m_build >> m_fingerprint; }
};
...
struct GP_Client_CharCreate : public GamePacket
{
    ...
    void pack(StlBuffer& buf) const override { buf << m_gender << m_portrait << m_name << m_classId; }
    void unpack(StlBuffer& buf) override { buf >> m_gender >> m_portrait >> m_name >> m_classId; }
};
...
struct GP_Client_DeleteCharacter : public GamePacket
{
    ...
    void pack(StlBuffer& buf) const override { buf << m_playerGuid; }
    void unpack(StlBuffer& buf) override { buf >> m_playerGuid; }
};
...
struct GP_Client_RequestMove : public GamePacket
{
    ...
    void pack(StlBuffer& buf) const override { buf << m_destX << m_destY << m_wasd; }
    void unpack(StlBuffer& buf) override { buf >> m_destX >> m_destY >> m_wasd; }
};

struct GP_Client_RequestStop : public GamePacket
{
    // Advisory client position at key release. The server never trusts this as
    // authoritative position; it is useful for prediction diagnostics only.
    float m_myX = 0, m_myY = 0;

    uint16_t getOpcode() const override { return Opcode::Client_RequestStop; }
    void pack(StlBuffer& buf) const override { buf << m_myX << m_myY; }
    void unpack(StlBuffer& buf) override { buf >> m_myX >> m_myY; }
};
...
struct GP_Client_CastSpell : public GamePacket
{
    ...
    float m_targetPosX = 0, m_targetPosY = 0;
    bool m_ctm = false;

    uint16_t getOpcode() const override { return Opcode::Client_CastSpell; }
    void pack(StlBuffer& buf) const override { buf << m_spellId << m_targetGuid << m_targetPosX << m_targetPosY << m_ctm; }
    void unpack(StlBuffer& buf) override { buf >> m_spellId >> m_targetGuid >> m_targetPosX >> m_targetPosY >> m_ctm; }
};
