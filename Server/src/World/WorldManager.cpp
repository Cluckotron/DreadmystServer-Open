...
void WorldManager::sendPlayerTo(Player* target, Player* playerToSend)
{
    ...
    // Send the same authoritative variable set used for the controller's own
    // Player packet.  This includes MoveSpeedPct, faction/combat state and stats,
    // so other clients animate and present the player consistently.
    packet.m_variables = playerToSend->getVariables();
